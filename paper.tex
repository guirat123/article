...
%  Page limit: 17 pages including references, tables and figures.
% =====================================================================
...
Verse-level repositories such as EveryAyah~\cite{everyayah} link recordings to
verses but provide no temporal Tajweed annotation. The Tarteel
dataset~\cite{khan2023tarteel} contributes about 25{,}000 verse recordings
(67.39\,h) from more than 1{,}200 speakers under realistic conditions, yet its
annotations link audio to text rather than to rule realisations.
Ar-DAD~\cite{ardad} (15{,}810 clips from 30 reciters plus 397 imitator
recordings) targets speaker identification; QDAT~\cite{qdat} provides expert
judgements on three rules from over 150 readers but with narrow coverage and
heterogeneous recordings; and a crowdsourced corpus of about 7{,}000 recitations
from 1{,}287 participants~\cite{quranic_audio_crowd} offers only category-level
labels (crowd accuracy 0.77, inter-rater agreement 0.63). Large-scale ASR
corpora---581{,}216 ayah recordings ($\approx$2{,}882\,h, 44
reciters)~\cite{quran_recitations_asr} and the 263{,}263-recording
Quran-Ayah-Corpus with speaker-disjoint partitions~\cite{quran_ayah_corpus}---
prioritise transcription over pronunciation correctness. Transmission-aware
resources are more recent: the Annotated Quranic Qira'at
Dataset~\cite{smail2024aqqd} adds qira'at and phonetic-variation metadata to
24{,}183 recordings from 309 reciters, and Riwaya-ID~\cite{riwaya_id} separates
Warsh from Hafs with 82\% accuracy using wav2vec~2.0~\cite{baevski2020wav2vec}
and Whisper~\cite{radford2023whisper}. Collectively these resources trade
lexical and speaker coverage against rule-level, temporally localised
supervision, so corpus size alone cannot satisfy the requirements of automatic
Tajweed recognition.

\subsection{Automatic Tajweed Recognition}

Acoustic Tajweed assessment began with phoneme-duration analysis of Madd and
Ghunnah from ten reciters~\cite{mohammed2018madd}, showing temporal cues to be
interpretable but rate-sensitive. Qalqalah has received particular attention,
from MFCC representations with vector quantisation~\cite{qalqalah_vq} to
multilayer perceptrons for Qalqalah Kubra~\cite{qalqalah_mlp}.
Smartajweed~\cite{alagrami2021smartajweed} cast recognition as several binary
acoustic problems (Idgham Meem, Ikhfaa Meem, Tafkheem Lam, Tarqeeq Lam) with
filter-bank features and radial-basis-function SVMs, reporting 99\% validation
accuracy under restricted conditions. Convolutional networks with MFCC inputs
reached 90.8\% validation accuracy for Qalqalah from 3{,}322 samples of four
professional reciters~\cite{omran2023cnn}, while recurrent models on QDAT
reported 77.0\%, 59.4\% and 66.1\% for Al-Madd al-Munfasil, Al-Ghunnah
al-Mushaddadah and Al-Ikhfaa~\cite{alahjal2023lstm}. Hybrid pipelines combine ASR
and forced alignment with a dedicated classifier: TajweedAI~\cite{tajweedai}
reached 100\% accuracy internally but 57.14\% externally, showing that
controlled-condition performance does not imply robust generalisation. Two
conclusions recur: Tajweed phenomena are not equally separable acoustically, and
accuracy alone is insufficient under class imbalance. Idgham and Qalqalah
nonetheless remain fragmented across datasets, features and protocols, with no
unified framework spanning multiple reciters and traditions.
...
assimilation, nasalisation or consonantal release.
...
bootstrap-trained trees over randomised feature subsets by majority vote,
...
training observations in feature space, serving as an instance-based baseline
sensitive to
...
Per-class results differ sharply (Table~\ref{tab:perclass}). Idgham bi-Ghunnah
is recognised almost perfectly by every classifier and Qalqalah Wusta remains
strong and architecturally stable, whereas Qalqalah Mushaddadah is intermediate
and Idgham bila-Ghunnah is the most difficult class; its errors are distributed
across several competitors rather than concentrated in one, indicating overlap
with multiple categories under the current representation. SVM's relatively high
recall for Idgham bila-Ghunnah explains its leading balanced accuracy despite
XGBoost's higher Macro-F1. PR analysis shows precision for Idgham bila-Ghunnah
falling steeply as recall increases, so its ROC-AUC must not be read as uniform
strength.

\begin{table}[t]
\centering
\caption{Per-class diagnostics. Recall ranges are taken across the five
classifiers; ROC-AUC values are those of XGBoost.}
\label{tab:perclass}
\begin{tabular}{lcc}
\hline
Class & Recall range (5 classifiers) & ROC-AUC (XGBoost) \\
\hline
Idgham bi-Ghunnah    & $\approx$1.00        & 1.00 \\
Qalqalah Wusta       & 0.88--0.92           & 0.99 \\
Qalqalah Mushaddadah & 0.58--0.72           & 0.89 \\
Idgham bila-Ghunnah  & 0.017--0.49          & 0.87 \\
Qalqalah Sughra      & 0.55 (XGBoost)       & 0.84 \\
\hline
\end{tabular}
\end{table}
...
Ablation confirms their causal contribution (Table~\ref{tab:ablation}): removing
the depth metric reduces Macro-F1 from 0.6935 to 0.5169 ($-0.1766$) and removing
duration to 0.5587 ($-0.1348$), while depth$+$duration alone retains 0.6424
against 0.2290 for nasality alone. The full set remains superior, so the
remaining descriptors contribute complementary information.

\begin{table}[t]
\centering
\caption{Feature ablation with XGBoost on the held-out test set.}
\label{tab:ablation}
\begin{tabular}{lrrr}
\hline
Configuration & Accuracy & Balanced acc. & Macro-F1 \\
\hline
All features          & 0.7202 & 0.6953 & 0.6935 \\
Remove nasality       & 0.6964 & 0.6580 & 0.6544 \\
Remove duration       & 0.5798 & 0.5572 & 0.5587 \\
Remove depth metric   & 0.5571 & 0.5222 & 0.5169 \\
Remove spectral       & 0.6905 & 0.6519 & 0.6512 \\
Nasality only         & 0.2393 & 0.2415 & 0.2290 \\
Depth $+$ duration only & 0.6571 & 0.6713 & 0.6424 \\
\hline
\end{tabular}
\end{table}
...
(Table~\ref{tab:balancing}). Unbalanced training yields the highest accuracy
...
\begin{table}[t]
\centering
\caption{Effect of class-balancing strategies (XGBoost). ``B.-Gh.'' denotes the
Idgham bila-Ghunnah class.}
\label{tab:balancing}
\begin{tabular}{lrrrrr}
\hline
Configuration & Accuracy & Balanced acc. & Macro-F1 & B.-Gh. recall & B.-Gh. F1 \\
\hline
Original          & 0.7357 & 0.6617 & 0.6592 & 0.0678 & 0.1111 \\
Class weighting   & 0.7202 & 0.6953 & 0.6935 & 0.3390 & 0.3478 \\
SMOTE             & 0.7119 & 0.7121 & 0.6951 & 0.4915 & 0.3742 \\
Borderline-SMOTE  & 0.7071 & 0.6899 & 0.6812 & 0.3559 & 0.3307 \\
SMOTE $+$ weighting & 0.7119 & 0.7121 & 0.6951 & 0.4915 & 0.3742 \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.40\textwidth]{fig3_cross_riwayah.pdf}
